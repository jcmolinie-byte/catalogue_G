...
import { 
  ...
  LayoutGrid, ScanLine, Home, StickyNote, Trash2, Plus, Pencil, ImageIcon, Share2, ShoppingCart, Minus, Wrench, Settings
} from 'lucide-react';
...
// --- GOOGLE SHEETS : "Publier sur le web" ---
// Pour obtenir ces URLs :
//   1. Ouvrir le fichier Excel dans Google Sheets
//   2. Fichier → Partager → Publier sur le web
//   3. Choisir "Document entier" + Format "Microsoft Excel (.xlsx)"
//   4. Cliquer "Publier" et coller l'URL ci-dessous
// 
// Remplacez VOTRE_ID_CATALOGUE et VOTRE_ID_EQUIPEMENTS par les identifiants de vos Google Sheets publiés.
// L'URL ressemble à : https://docs.google.com/spreadsheets/d/e/2PACX-XXXXX/pub?output=xlsx

const CATALOG_URL = 'https://docs.google.com/spreadsheets/d/e/VOTRE_ID_CATALOGUE/pub?output=xlsx';
const EQUIPMENTS_URL = 'https://docs.google.com/spreadsheets/d/e/VOTRE_ID_EQUIPEMENTS/pub?output=xlsx';
...
export default function App() {
      ...
      workbook.SheetNames.forEach(sheetName => {
        ...
        if (json && json.length > 0) {
          const sheetItems = json.map((row: any, index) => {
            ...
            // On cherche le poste technique avec les anciens noms OU "Sous_Zone"
            const equipmentId = String(getValue(['Equipement', 'équipement', 'poste technique', 'Sous_Zone']) || '');
            ...
            return {
              ...
              equipmentLabel: String(getValue(['Libellé équipement', 'libelle equipement', 'libelle', 'description equipement']) || ''),
  ...
  const fetchCatalog = async () => {
    try {
      ...
      // Synchro des équipements depuis Google Sheets
      try {
      ...
      } catch (err) { console.error("Erreur sync Equipements Google Sheets:", err); }
      ...
      if (importedItems) {
        setCatalogItems(importedItems);
        console.log(`${importedItems.length} articles synchronisés depuis Google Sheets !`);
      }
    } catch (err) {
        ...
        <AnimatePresence mode="wait">
          {view === 'home' && (
            ...
            >
              ...
              <div className="text-center mb-10">
                <h2 className="text-3xl font-bold text-white">Magasin Nesle</h2>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4 w-full max-w-md">
                ...
                <motion.button
                  ...
                  className="col-span-2 group relative bg-white dark:bg-slate-800 rounded-2xl border border-gray-200 dark:border-slate-700 p-6 flex flex-col items-center gap-3 shadow-sm hover:shadow-xl hover:border-amber-300 transition-all duration-300 overflow-hidden"
                >
          ...
          {view === 'equipments' && (
            <motion.div key="equipments" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="space-y-6">
              ...
              {!selectedEquipmentId ? (
                <div className="space-y-2">
                  {Array.from(new Set(equipments.filter(e => (e.equipment && e.equipment.toLowerCase().includes(equipmentSearchQuery.toLowerCase())) || (e.equipmentLabel && e.equipmentLabel.toLowerCase().includes(equipmentSearchQuery.toLowerCase()))).map(e => e.equipment)))
                    .slice(0, 50)
                    .map(eqName => {
                ...
                <div className="space-y-4">
                  ...
                  <h4 className="font-bold text-gray-900 dark:text-white pt-2 flex items-center gap-2"><Settings size={18} className="text-gray-400" /> Nomenclature ({equipments.filter(e => e.equipment === selectedEquipmentId).length})</h4>
