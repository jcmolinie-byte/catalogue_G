...
// BarcodeDetector API native - pas besoin de librairie externe
...
export default function App() {
  ...
  const codeReaderRef = useRef<any>(null); // référence à l'animation frame / BarcodeDetector
  ...
  const startCamera = async () => {
    try {
      // Attendre que le DOM monte l'élément vidéo
      let attempts = 0;
      while (!videoRef.current && attempts < 20) {
        await new Promise(resolve => setTimeout(resolve, 100));
        attempts++;
      }
      if (!videoRef.current) throw new Error("Élément vidéo non trouvé dans le DOM.");
      ...
      // Utiliser BarcodeDetector natif (Android Chrome 83+) si disponible
      // Sinon fallback sur ZXing
      ...
      if (BarcodeDetectorAPI) {
        ...
        // Passer le stream existant directement
  ...
  const stopCamera = () => {
    // Annuler la boucle (requestAnimationFrame ou ZXing)
    if (codeReaderRef.current) {
      if (typeof codeReaderRef.current === 'number') {
        cancelAnimationFrame(codeReaderRef.current);
      } else if (typeof codeReaderRef.current.reset === 'function') {
        codeReaderRef.current.reset();
      }
      codeReaderRef.current = null;
    }
    // Libérer le stream caméra
