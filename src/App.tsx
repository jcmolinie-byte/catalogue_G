...
export default function App() {
    ...
    if (Array.isArray(ai.specs)) {
      ai.specs.forEach(spec => {
        ...
        if (specClean.includes('kw')) {
          const kwValue = specNoSpace.replace('kw', '');
          if (kwValue && (itemNameClean.includes(kwValue + 'kw') || itemNameClean.includes(kwValue + ' kw'))) {
            score += 1500; // Un match sur la puissance est prioritaire
          }
        }
  ...
  const analyzePhoto = async () => {
    ...
    try {
      ...
      let mistralKey = localStorage.getItem('mistral_api_key');
      if (!mistralKey) {
        setView('settings');
      ...
      const canvas = document.createElement('canvas');
      const maxSize = 1200; // Augmenté pour l'OCR
        ...
        body: JSON.stringify({
          ...
          messages: [{
            ...
            content: [
              {
                type: 'text',
                text: `Analyse cette plaque technique industrielle. 
                1. Retranscris d'abord TOUT le texte visible (OCR).
                2. Extrais ensuite :
                - raw_ocr: texte brut (TRÈS IMPORTANT : tout sur une seule ligne sans retour chariot)
                - type: moteur, pompe, etc.
                - brand: marque
                - model: reference
                - specs: TOUTES les valeurs techniques (KW, RPM, V, A, Hz...) - Identifie clairement la puissance en kW.
                - description: description courte (ex: Moteur 15kW)
      ...
      // Extraction robuste du JSON (recherche du premier { et dernier })
      const firstBrace = textContent.indexOf('{');
      const lastBrace = textContent.lastIndexOf('}');
      
      if (firstBrace === -1 || lastBrace === -1) {
        throw new Error(`Réponse IA invalide (pas de JSON trouvé) : ${textContent.substring(0, 100)}`);
      }
      
      const jsonContent = textContent.substring(firstBrace, lastBrace + 1);

      // Nettoyage des caractères de contrôle invisibles qui peuvent casser le JSON
      const sanitized = jsonContent.replace(/[\u0000-\u001F\u007F-\u009F]/g, (match) => {
        return match === '\n' || match === '\r' || match === '\t' ? match : '';
      });
      ...
      try {
        ...
        throw new Error(`Réponse IA non parsable : ${sanitized.substring(0, 150)}`);
      }
      ...
      if (scored.length === 1 && scored[0].score >= 500) {
        ...
        setAiMatches(scored);
        setScanResult(result.raw_ocr || `${result.brand ? result.brand + ' — ' : ''}${result.specs?.join(', ')}`);
      }
    } catch (err: any) {
      console.error(err);
              ...
              <div className="bg-white dark:bg-slate-800 rounded-3xl p-6 border border-gray-200 dark:border-slate-700 shadow-sm">
                <div className="flex items-center gap-3 mb-6">
                  ...
                  <div>
                    ...
                    <p className="text-[11px] text-gray-400">Propulsé par Mistral AI (Pixtral)</p>
                  </div>
                </div>

                <div className="space-y-4">
                  <div>
                    <label className="block text-xs font-bold text-gray-400 uppercase mb-1.5 px-1">Clé API Mistral</label>
                    <input 
                      type="password" 
                      placeholder="Saisir votre clé api..."
                      defaultValue={localStorage.getItem('mistral_api_key') || ''}
                      onChange={(e) => {
                        localStorage.setItem('mistral_api_key', e.target.value);
                      }}
                      className="w-full px-4 py-3 bg-gray-50 dark:bg-slate-900 border border-gray-200 dark:border-slate-700 rounded-xl outline-none focus:border-orange-500 transition-all font-mono text-sm dark:text-white"
                    />
                  </div>
                  <p className="text-[10px] text-gray-400 px-1 leading-relaxed">
                    Votre clé est stockée uniquement sur cet appareil. Vous pouvez l'obtenir sur le site console.mistral.ai.
                  </p>
                </div>
              </div>
