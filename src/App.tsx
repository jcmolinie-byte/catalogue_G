...
interface CompanionIntent {
  intent: 'stock_search' | 'equipment_search' | 'equipment_list' | 'technical_info' | 'unknown';
  itemTerms: string[];
...
export default function App() {
  ...
  const companionStopWords = new Set([
    ...
    'caracteristiques', 'caracteristique', 'article', 'articles', 'poste', 'postes', 'technique', 'techniques', 'equipement', 'equipements', 'ref',
    'reference', 'sap', 'est', 'ce', 'cet', 'cette', 'avons', 'nous', 'magasin', 'stock',
    'disponible', 'disponibles', 'avoir', 'avez', 'as', 'tu', 'il', 'y', 'a', 'liste', 'lister',
    'tous', 'toutes', 'affiche', 'afficher'
  ]);
  ...
  const expandCompanionTerm = (term: string) => {
    ...
    if (token === 'colle' || token === 'colles') ['colle', 'silicone', 'adhesif', 'mastic', 'cartouche'].forEach(v => variants.add(v));
  ...
  const buildFallbackCompanionIntent = (question: string): CompanionIntent => {
    ...
    const wantsEquipmentList = /\b(liste|lister|affiche|afficher|tous|toutes)\b/.test(normalizedQuestion) &&
      /\b(poste|postes|equipement|equipements|technique|techniques)\b/.test(normalizedQuestion);
    ...
    return {
      intent: wantsEquipmentList ? 'equipment_list' : (/caracteristique|fiche|technique|manuel|datasheet|internet|web/i.test(normalizedQuestion) ? 'technical_info' : 'stock_search'),
      itemTerms: wantsEquipmentList ? [] : itemTerms,
      requiredTerms: wantsEquipmentList ? [] : itemTerms.slice(0, 4),
      optionalTerms: [],
...
- Pour "liste tous les postes techniques", intent doit etre "equipment_list" et requiredTerms doit etre vide.
...
- Ne devine pas un article si les concepts obligatoires ne sont pas presents.`
      ...
      const intent: CompanionIntent = {
        intent: ['stock_search', 'equipment_search', 'equipment_list', 'technical_info', 'unknown'].includes(parsed.intent) ? parsed.intent : fallback.intent,
      ...
      if (fallback.requiredTerms.length > 0 && intent.intent !== 'equipment_list') {
        intent.requiredTerms = Array.from(new Set([...fallback.requiredTerms, ...intent.requiredTerms]));
        intent.itemTerms = Array.from(new Set([...fallback.itemTerms, ...intent.itemTerms]));
      }
  ...
  const findCompanionResults = (question: string, intent: CompanionIntent): CompanionResult[] => {
    ...
    if (intent.intent === 'equipment_list') {
      const seen = new Set<string>();
      return companionSearchData.equipmentRows
        .filter(({ part }) => {
          if (!part.equipment || seen.has(part.equipment)) return false;
          seen.add(part.equipment);
          return true;
        })
        .slice(0, 20)
        .map(({ part }) => ({
          id: `equipment-${part.equipment}`,
          score: 100,
          title: part.equipment,
          subtitle: part.equipmentLabel || 'Poste technique',
  ...
  const buildFallbackCompanionAnswer = (question: string, results: CompanionResult[], intent?: CompanionIntent) => {
    ...
    if (intent?.intent === 'equipment_list') {
      return `J'ai trouvé ${results.length} postes techniques à afficher. Voici les premiers résultats disponibles.`;
    }
...
Si intent vaut "equipment_list", reponds que tu affiches les postes techniques trouves, sans parler d'article SAP.
