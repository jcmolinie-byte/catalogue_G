...
const HighlightedText = ({ text, highlight }: { text: string; highlight: string }) => {
  ...
  const wordsNorm = words.map(w => strip(w));
  
  // Trouver tous les segments à surligner
  let matches: {start: number, end: number}[] = [];
  wordsNorm.forEach(word => {
    let pos = textNorm.indexOf(word);
    while (pos !== -1) {
      matches.push({ start: pos, end: pos + word.length });
      pos = textNorm.indexOf(word, pos + 1);
    }
  });
          ...
          {view === 'home' && (
            ...
            >
              ...
              <div className="text-center mb-10">
                ...
                <p className="text-gray-400 text-sm">Catalogue SAP & Outils</p>
              </div>
