...
export default function App() {
  ...
  const toggleVoiceSearch = () => {
    ...
    // Intégration de la grammaire à partir du catalogue
    if (SpeechGrammarList) {
      const words = catalogItemsRef.current
        .map(i => i.name.split(/[\s,.-]+/))
        .flat()
        .filter(w => w.length > 2)
        .map(w => w.toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, ''))
        .filter((v, i, a) => a.indexOf(v) === i && /^[a-z]+$/.test(v));
    ...
    recognition.maxAlternatives = 5; // On demande plus d'essais au système
    ...
    recognition.onresult = (event: any) => {
      ...
      setVoiceDebug(`Entendu: "${transcript}" (${confidence}%)`);
    ...
    recognition.onnomatch = (event: any) => {
      setVoiceDebug('Le système a entendu un son mais ne l\'a pas reconnu comme un mot.');
    };
