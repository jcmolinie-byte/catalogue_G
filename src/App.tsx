...
export default function App() {
  ...
  const handleRequestRestock = async (item: CatalogItem) => {
    try {
      ...
      if (!response.ok) throw new Error('Erreur réseau EmailJS');
      
      alert('Demande de réapprovisionnement envoyée avec succès au magasin !');
            ...
            >
              ...
              <div className="w-full flex justify-end mb-4">
                <button
                  ...
                  disabled={isSyncing}
                  className={cn(
                    "p-2.5 bg-slate-800 border border-slate-700 rounded-xl transition-all active:scale-90 flex items-center gap-2",
                    isSyncing ? "text-yellow-400 border-yellow-400/50" : "text-gray-400 hover:text-white hover:border-slate-500"
                  )}
                  title="Synchroniser avec GitHub"
                >
                  <Zap size={20} className={isSyncing ? "animate-pulse" : ""} />
                  {isSyncing && <span className="text-xs font-medium">Sync...</span>}
                </button>
                <button
