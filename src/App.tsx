...
const GITHUB_CATALOG_URL = 'https://raw.githubusercontent.com/jcmolinie-byte/catalogue_G/main/public/catalogue.xlsx';
const GITHUB_EQUIPMENTS_URL = 'https://raw.githubusercontent.com/jcmolinie-byte/catalogue_G/main/public/catalogue_postes_techniques_nesle.xlsx';
...
export default function App() {
  ...
  const parseEquipmentsExcelData = (data: any) => {
    try {
      ...
      return json.map((row: any, index) => {
        ...
        return {
          id: `equip-${index}-${Date.now()}`,
          equipment: String(getValue(['Equipement', 'équipement', 'poste technique']) || ''),
  ...
  const fetchGitHubCatalog = async () => {
    try {
      ...
      // Synchro des équipements
      try {
        const eqResp = await fetch(GITHUB_EQUIPMENTS_URL);
        if (eqResp.ok) {
