...
import { 
  ...
  LayoutGrid, ScanLine, Home, StickyNote, Trash2, Plus, Pencil, ImageIcon, Share2, ShoppingCart, Minus, Wrench, Settings, Sun, Moon, Sparkles, ClipboardList
} from 'lucide-react';
...
export default function App() {
  ...
  // Fonction utilitaire : récupère TOUTES les lignes d'une table par lots de 1000
  ...
  const fetchGitHubCatalog = async () => {
    try {
      ...
      // Sync équipements depuis Supabase (toutes les lignes)
      try {
        const equipsData = await fetchAllRows('equipments');
      ...
      // Sync catalogue depuis Supabase (toutes les lignes)
      const catalogData = await fetchAllRows('catalog_items');
  ...
  // --- ANALYSE PHOTO AVEC GEMINI (VISION) + FALLBACK GROQ ---

  // Prompt commun aux deux fournisseurs
...
Réponds uniquement par le JSON.`;
  ...
  const parseAIResponse = (textContent: string): AIAnalysis => {
    ...
    try {
      ...
      try {
        result = JSON.parse(sanitized.replace(/\\/g, "\\\\"));
      } catch {
  ...
  const callGemini = async (base64Data: string, apiKey: string): Promise<string> => {
    ...
    if (!response.ok) {
      let errMsg = `Erreur Google ${response.status}`;
      try {
  ...
  const callGroq = async (base64Data: string, apiKey: string): Promise<string> => {
    ...
    if (!response.ok) {
      let errMsg = `Erreur Groq ${response.status}`;
      try {
        const errData = await response.json();
        if (errData.error?.message) errMsg = errData.error.message;
      } catch {}
  ...
  const analyzePhoto = async () => {
    ...
    try {
      ...
      // --- ORCHESTRATION : Gemini d'abord, Groq en fallback sur quota / panne ---
  ...
  const categories = useMemo(() => Array.from(new Set(catalogItems.map(i => i.category))).filter(c => c && c.trim() && c !== 'Non spécifié'), [catalogItems]);
      ...
      <main className="max-w-5xl mx-auto p-4 pb-24">
        <AnimatePresence mode="wait">
          {view === 'home' && (
            ...
            >
              {/* Greeting */}
              <div className="w-full max-w-md flex justify-center items-center mb-10 px-2">
              ...
              <div className="grid grid-cols-2 gap-4 w-full max-w-md">
                ...
                {/* Ligne 5 : Inventaire (Pleine largeur) */}
                <motion.button
                  whileHover={{ scale: 1.02, y: -2 }}
                  whileTap={{ scale: 0.98 }}
                  onClick={() => setView('inventaire')}
                  className="col-span-2 group relative bg-white dark:bg-slate-800 rounded-2xl border border-gray-200 dark:border-slate-700 p-6 flex items-center justify-center gap-6 shadow-sm hover:shadow-xl hover:border-emerald-300 transition-all duration-300 overflow-hidden"
                >
                  <div className="absolute inset-0 bg-gradient-to-br from-emerald-500/5 to-emerald-600/10 opacity-0 group-hover:opacity-100 transition-opacity duration-300" />
                  <div className="relative w-14 h-14 bg-emerald-50 rounded-2xl flex items-center justify-center text-emerald-600 group-hover:bg-emerald-600 group-hover:text-white transition-colors duration-300 shadow-sm">
                  ...
                  <div className="relative">
                    <p className="font-bold text-gray-900 dark:text-white text-base">Inventaire</p>
                    <p className="text-xs text-gray-400 dark:text-gray-400 mt-0.5">Comptage par rayon</p>
                  </div>
                </motion.button>
              </div>
          ...
          {view === 'inventaire' && (
            <Inventaire onRetour={() => setView('home')} />
          )}
              ...
              {!selectedEquipmentId ? (
                <div className="space-y-2">
                  {Array.from(new Set(equipments.filter(e => {
                    if (!e.equipment || !e.equipment.trim()) return false;
            ...
            <motion.div key="settings" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-8 max-w-lg mx-auto">
              ...
              <div className="bg-white dark:bg-slate-800 rounded-3xl p-6 border border-gray-200 dark:border-slate-700 shadow-sm">
                ...
                <div className="space-y-4">
                  ...
                  <div className="mt-4">
                    <label className="block text-xs font-bold text-gray-400 uppercase mb-1.5 px-1">Clé API ElevenLabs <span className="text-purple-500 normal-case font-normal">(voix naturelle pour le chatbot)</span></label>
                    <input
                      type="password"
                      placeholder="Saisir votre clé api ElevenLabs..."
                      defaultValue={localStorage.getItem('elevenlabs_api_key') || ''}
                      onChange={(e) => localStorage.setItem('elevenlabs_api_key', e.target.value)}
                      className="w-full px-4 py-3 bg-gray-50 dark:bg-slate-900 border border-gray-200 dark:border-slate-700 rounded-xl outline-none focus:border-purple-500 transition-all font-mono text-sm dark:text-white"
                    />
                    <p className="text-[10px] text-gray-400 px-1 mt-1.5 leading-relaxed">
                      Obtenez une clé gratuite sur <a href="https://elevenlabs.io/" target="_blank" rel="noopener noreferrer" className="text-purple-500 underline">elevenlabs.io</a>. ~10 000 caractères/mois gratuits. Sans clé, le chatbot utilise la voix du navigateur.
                    </p>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-white dark:bg-slate-800 rounded-3xl p-6 border border-gray-200 dark:border-slate-700 shadow-sm">
                ...
                <div className="flex gap-3">
                  <button
                    ...
                    disabled={isSyncing}
                    className={cn(
                      "flex-1 flex items-center justify-center gap-2 py-3 rounded-2xl border font-medium text-sm transition-all active:scale-95",
                      isSyncing
                        ? "bg-yellow-50 dark:bg-yellow-900/20 border-yellow-400 text-yellow-600"
                        : "bg-gray-50 dark:bg-slate-900 border-gray-200 dark:border-slate-700 text-gray-600 dark:text-gray-300 hover:border-blue-400 hover:text-blue-600"
                    )}
                  >
                  ...
                  <button
                    ...
                    className="flex-1 flex items-center justify-center gap-2 py-3 rounded-2xl border font-medium text-sm bg-gray-50 dark:bg-slate-900 border-gray-200 dark:border-slate-700 text-gray-600 dark:text-gray-300 hover:border-blue-400 hover:text-blue-600 transition-all active:scale-95"
                  >
                    <FileUp size={16} />
                    Importer Excel
                  </button>
