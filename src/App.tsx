...
{"type":"type de pièce (ex: moteur, reducteur, pompe)","brand":"marque fabricant","model":"reference modele exact (sans la marque)","specs":["liste des valeurs techniques avec leur unité (ex: '0.75kW', '230V', '1400rpm', '50Hz', 'IP55'). Ne mets QUE les valeurs et unités, SANS mots texte comme 'puissance' ou 'tension'."],"description":"resume court en français de la plaque"}`
  ...
  return (
    <div className={cn("min-h-screen bg-[#F8F9FA] dark:bg-slate-900 text-gray-900 dark:text-white transition-colors duration-300", darkMode && "dark")}>
          ...
          {view === 'home' && (
            ...
            >
              ...
              <button
                ...
                className="mb-8 flex items-center gap-2 px-5 py-2.5 bg-white dark:bg-slate-800 border border-gray-200 dark:border-slate-700 rounded-full shadow-sm hover:shadow-md active:scale-95 transition-all"
              >
                {darkMode ? (
                  <><Sun size={18} className="text-amber-500" /><span className="text-sm font-medium text-gray-600 dark:text-gray-300">Mode clair</span></>
                ) : (
                  <><Moon size={18} className="text-indigo-500" /><span className="text-sm font-medium text-gray-600 dark:text-gray-300">Mode sombre</span></>
                )}
              </button>
                ...
                {filteredItems.slice(0, itemsLimit).map(item => (
                  <div key={item.id} onClick={() => setSelectedItem(item)} className="bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 p-4 flex items-center justify-between active:scale-[0.98] transition-transform cursor-pointer group">
                    <div className="flex-1 min-w-0 pr-4">
                      <h3 className="font-bold text-gray-900 dark:text-white line-clamp-2 group-hover:text-blue-600">{item.name}</h3>
