...
export default function App() {
  ...
  const scoreItem = (item: CatalogItem, ai: AIAnalysis): number => {
    // Fonction qui découpe un texte en liste de mots importants (alphanumériques purs)
    const extractWords = (text: string) => {
      ...
      return String(text)
        ...
        .split(/[\s_/\-]+/) // Découpe par espaces, tirets, slashes...
        .map(w => w.replace(/[^a-z0-9.]/g, '')) // Garde juste l'alphanumérique
        .filter(w => w.length >= 2 && !['non', 'null', 'none', 'inconnu', 'n/a', 'na', 'nr', 'nc'].includes(w));
    };

    // Les mots clés réels de l'article dans le catalogue
    const itemWords = [...extractWords(item.name), ...extractWords(item.sapCode), ...extractWords(item.category)];
    
    let score = 0;

    const matchTermWords = (term: string | undefined, points: number) => {
      if (!term) return;
      const aiWords = extractWords(term);
      aiWords.forEach(aiw => {
        // Si le mot est exactement dans le catalogue
        if (itemWords.includes(aiw)) {
          score += points;
        } else {
          // Sinon correspondance partielle (ex: '0.75' contenu dans '0.75kw')
          const isPartial = itemWords.some(iw => iw.includes(aiw) || aiw.includes(iw));
          if (isPartial) {
             score += Math.floor(points / 2); // 50% des points pour une inclusion partielle
          }
        }
      });
    };

    // On attribue des poids plus intelligents point par point :
    matchTermWords(ai.model, 300);    // Très forte valeur au modèle (ex: DT71D4)
    matchTermWords(ai.brand, 100);    // Marque
    matchTermWords(ai.type, 50);      // Type large (ex: Moteur, Pompe)
    
    if (Array.isArray(ai.specs)) {
      ai.specs.forEach(spec => matchTermWords(spec, 200)); // Puissance, voltage, etc.
    }

    // BONUS : Si jamais le code SAP est explicitement visible sur la plaque
    if (item.sapCode && item.sapCode !== 'N/A') {
      const sapNorm = extractWords(item.sapCode).join('');
      if (sapNorm.length >= 5) { // Uniquement si le SAP est assez long
        const aiFullText = extractWords(`${ai.model} ${ai.description} ${(ai.specs || []).join(' ')}`).join('');
        if (aiFullText.includes(sapNorm)) {
          score += 5000;
        }
      }
    }
