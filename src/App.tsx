...
import { 
  ...
  LayoutGrid, ScanLine, Home, StickyNote, Trash2, Plus, Pencil, ImageIcon
} from 'lucide-react';
...
const HighlightedText = ({ text, highlight }: { text: string; highlight: string }) => {
  ...
  return (
    <>
      {parts.map((part, i) => (
        pattern.test(part) ? 
          <span key={i} className="text-yellow-400 font-extrabold">{part}</span> : 
        ...
        <AnimatePresence mode="wait">
              ...
              <div className="grid grid-cols-2 gap-4 w-full max-w-md">
                ...
                >
                  ...
                  <div className="relative text-center">
                    ...
                    <p className="text-[11px] text-gray-400 dark:text-gray-400 mt-0.5">Recherche SAP</p>
                  </div>
                </motion.button>
                ...
                >
                  ...
                  <div className="relative text-center">
                    ...
                    <p className="text-[11px] text-gray-400 dark:text-gray-400 mt-0.5">Identification technique</p>
                  </div>
                </motion.button>
                ...
                <motion.button
                  ...
                  className="group relative bg-white dark:bg-slate-800 rounded-2xl border border-gray-200 dark:border-slate-700 p-6 flex flex-col items-center gap-3 shadow-sm hover:shadow-xl hover:border-slate-300 transition-all duration-300 overflow-hidden"
                >
                  ...
                  <div className="relative w-14 h-14 bg-slate-50 rounded-2xl flex items-center justify-center text-slate-600 group-hover:bg-slate-600 group-hover:text-white transition-colors duration-300 shadow-sm">
                    <FileUp size={26} />
                  </div>
                  <div className="relative text-center">
                    <p className="font-bold text-gray-900 dark:text-white text-sm">Importer</p>
                    <p className="text-[11px] text-gray-400 dark:text-gray-400 mt-0.5">Catalogue Excel</p>
                  </div>
                </motion.button>
          ...
          {view === 'photo-preview' && pendingPhoto && (
            <motion.div key="photo-preview" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="fixed inset-0 z-[60] bg-black flex flex-col">
              <div className="p-4 flex items-center justify-center text-white z-10">
                <h2 className="font-bold">Aperçu de la photo</h2>
              </div>

              <div className="flex-1 relative flex items-center justify-center overflow-hidden p-4">
                <img src={pendingPhoto} alt="Aperçu" className="max-w-full max-h-full object-contain rounded-2xl shadow-2xl" />
              </div>

              <div className="p-6 bg-slate-900/90 backdrop-blur-md border-t border-slate-800 flex gap-3">
                <button 
                  onClick={() => {
                    setPendingPhoto(null);
                    setView('home');
                  }}
                  className="flex-1 py-4 bg-slate-800 text-gray-300 rounded-2xl font-bold active:scale-95 transition-all text-sm"
                >
                  Annuler
                </button>
                <button 
                  onClick={() => {
                    ...
                    // On ne réinitialise le titre que si on n'est pas déjà en train d'éditer
                  ...
                  className="flex-[2] flex items-center justify-center gap-2 py-4 bg-amber-500 text-white rounded-2xl font-bold shadow-xl active:scale-95 transition-all"
                >
