    ...
    try {
      ...
      if (view === 'scan') {
        ...
        if (BarcodeDetectorAPI && (await BarcodeDetectorAPI.getSupportedFormats()).includes('code_128')) {
          ...
          // Fallback ZXing via canvas — compatible iOS Safari et Android
          const { BrowserMultiFormatReader } = await import('@zxing/browser');
          const reader = new BrowserMultiFormatReader();
          zxingReaderRef.current = reader;
          const canvas = document.createElement('canvas');
          const ctx = canvas.getContext('2d', { willReadFrequently: true });
          let debugEl = document.getElementById('scan-debug');
          if (!debugEl) {
            debugEl = document.createElement('div');
            debugEl.id = 'scan-debug';
            debugEl.style.cssText = 'position:fixed;top:80px;left:10px;right:10px;background:rgba(0,0,0,0.8);color:#fff;font-size:11px;padding:8px;border-radius:8px;z-index:9999;word-break:break-all;max-height:120px;overflow:auto';
            document.body.appendChild(debugEl);
          }
          const log = (msg: string) => { if(debugEl) debugEl.innerHTML = msg; };
          log('ZXing canvas init...');
          const scan = async () => {
            let frameCount = 0;
            while (isActive && videoRef.current && ctx) {
              try {
                ...
                if (video.readyState >= 2 && video.videoWidth > 0) {
                  ...
                  log(`Frame ${frameCount} — ${canvas.width}x${canvas.height} — readyState:${video.readyState}`);
                  ...
                  log(`Frame ${frameCount} — vidéo pas prête: readyState=${video.readyState} w=${video.videoWidth}`);
                }
              } catch (e: any) {
                const msg = e?.message || String(e);
                if (!msg.includes('NotFoundException') && !msg.includes('No MultiFormat')) {
                  log(`ERREUR frame ${frameCount}: ${msg}`);
                }
              }
