...
// Scan via BarcodeDetector natif (Android Chrome) ou ZXing en fallback
...
export default function App() {
  ...
  const categories = useMemo(() => {
    ...
    return Array.from(cats).filter(cat => cat !== 'Général' && cat !== 'Non spécifié');
  }, [catalogItems]);
  ...
  const startCamera = async () => {
    ...
    try {
      ...
      if (!videoRef.current) throw new Error("Élément vidéo non trouvé.");
      ...
      // 4. Attendre que la vidéo ait des dimensions réelles
      ...
      // 5. Choisir le moteur de détection
      const BarcodeDetectorAPI = (window as any).BarcodeDetector;
      let useBarcodeDetector = false;

      if (BarcodeDetectorAPI) {
        try {
          const supported: string[] = await BarcodeDetectorAPI.getSupportedFormats();
          useBarcodeDetector = supported.includes('code_128') || supported.includes('CODE_128');
        } catch (_) {
          useBarcodeDetector = true; // tenter quand même
        }
      }

      if (useBarcodeDetector) {
        // --- Chemin A : BarcodeDetector natif (Android Chrome 83+) ---
        const detector = new BarcodeDetectorAPI({ formats: ['code_128', 'code_39', 'qr_code', 'ean_13'] });
        let scanning = true;
        ...
        const scanLoop = () => {
          ...
          if (video.readyState >= 2 && video.videoWidth > 0) {
            // detect() accepte directement un HTMLVideoElement — pas besoin de canvas
            detector.detect(video)
              .then((barcodes: any[]) => {
                if (barcodes.length > 0) {
                  ...
                  codeReaderRef.current = requestAnimationFrame(scanLoop);
                }
              })
              .catch(() => {
                codeReaderRef.current = requestAnimationFrame(scanLoop);
        ...
        codeReaderRef.current = { type: 'raf', id: requestAnimationFrame(scanLoop), stop: () => { scanning = false; } };

      } else {
        // --- Chemin B : ZXing (fallback iPhone / Firefox) ---
        ...
        codeReaderRef.current = { type: 'zxing', reader };
      }
  ...
  const stopCamera = () => {
    if (codeReaderRef.current) {
      try {
        if (codeReaderRef.current.type === 'raf') {
          codeReaderRef.current.stop();
          cancelAnimationFrame(codeReaderRef.current.id);
        } else if (codeReaderRef.current.type === 'zxing') {
                ...
                {displayedItems.map(item => (
                  ...
                  >
                    <div className="flex-1 min-w-0 pr-4">
                      {item.category && item.category !== 'Non spécifié' && (
                        <div className="flex items-center gap-2 mb-1">
                          <span className="text-[10px] font-bold text-blue-600 uppercase tracking-widest">{item.category}</span>
                        </div>
                      )}
                      <h3 className="font-bold text-gray-900 group-hover:text-blue-600 transition-colors truncate">{item.name}</h3>
          ...
          <div className="fixed inset-0 z-[70] flex items-end sm:items-center justify-center p-0 sm:p-4">
            ...
            >
              <div className="p-6 border-b border-gray-100 flex items-center justify-between bg-blue-600 text-white">
                <div>
                  {selectedItem.category && selectedItem.category !== 'Non spécifié' && (
              ...
              <div className="p-6 space-y-6">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  ...
                  <div className="bg-gray-50 p-4 rounded-2xl border border-gray-100">
                    <p className="text-[10px] font-bold text-gray-400 uppercase tracking-wider mb-1">Dernière Sortie</p>
                    <p className="font-bold text-gray-900">{selectedItem.lastExitDate || 'N/A'}</p>
                  </div>
                </div>
