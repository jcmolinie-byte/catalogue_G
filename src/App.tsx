          ...
          messages: [
            {
              ...
              content: [
                ...
                {
                  type: 'text',
                  text: `Tu es un assistant magasin industriel. Analyse cette photo de pièce et identifie sa nature (roulement, courroie, vanne, moteur, joint, filtre, etc.).

Voici le catalogue SAP (format CODE_SAP|DESIGNATION) :
${itemNames}

Retourne UNIQUEMENT un JSON valide, sans texte autour, sans backticks :
{
  "description": "description courte de la pièce photographiée",
  "matches": [
            ...
            <motion.div key="scan" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="fixed inset-0 z-[60] bg-black flex flex-col">
              ...
              {aiMatches.length > 1 && (
                <div className="absolute inset-0 bg-black/70 flex items-end z-10">
                  <div className="w-full bg-white rounded-t-3xl p-5 max-h-[70vh] overflow-y-auto">
                    <div className="flex justify-between items-center mb-3">
                      <div>
                        <p className="text-[11px] text-gray-400 font-medium">Reconnaissance IA</p>
                        <h3 className="font-bold text-gray-900">{scanResult?.replace('IA : ', '')}</h3>
                      </div>
                      <button onClick={() => { setAiMatches([]); setScanResult(null); }} className="p-2 bg-gray-100 rounded-full"><X size={18} /></button>
                    </div>
                    <p className="text-xs text-gray-500 mb-3">{aiMatches.length} articles SAP correspondants :</p>
                    <div className="space-y-2">
                      {aiMatches.map(item => (
                        <button
                          key={item.id}
