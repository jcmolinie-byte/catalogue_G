...
import { 
  ...
  LayoutGrid, ScanLine, Home, StickyNote, Trash2, Plus, Pencil, Sun, Moon
} from 'lucide-react';
...
export default function App() {
  ...
  const [darkMode, setDarkMode] = useState(() => {
    try {
      return localStorage.getItem('nesle_dark_mode') === 'true';
    } catch { return false; }
  });

  useEffect(() => {
    localStorage.setItem('nesle_dark_mode', String(darkMode));
  }, [darkMode]);
...
{"type":"type de pièce (ex: moteur, reducteur, pompe)","brand":"marque fabricant","model":"reference modele exact (sans la marque)","specs":["liste des valeurs techniques avec leur unité (ex: '0.75kW', '230V', '1400rpm', '50Hz', 'IP55'). Ne mets QUE les valeurs et unités, SANS mots texte comme 'puissance' ou 'tension'."],"description":"resume court en français de la plaque"}`
  ...
  return (
    <div className={cn("min-h-screen bg-[#F8F9FA] text-[#1A1A1A] transition-colors duration-300", darkMode && "dark")}>
          ...
          {view === 'home' && (
            ...
            >
              ...
              <div className="text-center mb-10">
                ...
                <h2 className="text-2xl font-bold text-gray-900 mb-1">Magasin Nesle</h2>
                <p className="text-gray-400 text-sm">Gestion des pièces & catalogue SAP</p>
              </div>

              {/* Dark mode toggle */}
              <button
                onClick={() => setDarkMode(d => !d)}
                className="mb-8 flex items-center gap-2 px-5 py-2.5 bg-white border border-gray-200 rounded-full shadow-sm hover:shadow-md active:scale-95 transition-all"
              >
                {darkMode ? (
                  <><Sun size={18} className="text-amber-500" /><span className="text-sm font-medium text-gray-600">Mode clair</span></>
                ) : (
                  <><Moon size={18} className="text-indigo-500" /><span className="text-sm font-medium text-gray-600">Mode sombre</span></>
                )}
              </button>
              ...
              <div className="grid grid-cols-2 gap-4 w-full max-w-md">
                ...
                <motion.button
                  ...
                  className="group relative bg-white rounded-2xl border border-gray-200 p-6 flex flex-col items-center gap-3 shadow-sm hover:shadow-xl hover:border-blue-300 transition-all duration-300 overflow-hidden"
                >
                  ...
                  <div className="relative text-center">
                    ...
                    <p className="text-[11px] text-gray-400 mt-0.5">Rechercher des pièces</p>
                  </div>
                </motion.button>
                ...
                <motion.button
                  ...
                  className="group relative bg-white rounded-2xl border border-gray-200 p-6 flex flex-col items-center gap-3 shadow-sm hover:shadow-xl hover:border-purple-300 transition-all duration-300 overflow-hidden"
                >
                  ...
                  <div className="relative text-center">
                    <p className="font-bold text-gray-900 text-sm">Scanner IA</p>
                    <p className="text-[11px] text-gray-400 mt-0.5">Identifier une pièce</p>
                  </div>
                </motion.button>
                ...
                <motion.button
                  ...
                  className="group relative bg-white rounded-2xl border border-gray-200 p-6 flex flex-col items-center gap-3 shadow-sm hover:shadow-xl hover:border-orange-300 transition-all duration-300 overflow-hidden"
                >
                  ...
                  <div className="relative text-center">
                    <p className="font-bold text-gray-900 text-sm">Rappels SAP</p>
                    <p className="text-[11px] text-gray-400 mt-0.5">{reminders.length} en attente</p>
                  </div>
                </motion.button>
                ...
                <motion.button
                  ...
                  className="group relative bg-white rounded-2xl border border-gray-200 p-6 flex flex-col items-center gap-3 shadow-sm hover:shadow-xl hover:border-emerald-300 transition-all duration-300 overflow-hidden"
                >
                  ...
                  <div className="relative text-center">
                    <p className="font-bold text-gray-900 text-sm">Importer</p>
                    <p className="text-[11px] text-gray-400 mt-0.5">Charger un fichier Excel</p>
                  </div>
                </motion.button>
                ...
                <motion.button
                  ...
                  className="group relative bg-white rounded-2xl border border-gray-200 p-6 flex flex-col items-center gap-3 shadow-sm hover:shadow-xl hover:border-amber-300 transition-all duration-300 overflow-hidden col-span-2"
                >
                  ...
                  <div className="relative text-center">
                    <p className="font-bold text-gray-900 text-sm">Bloc-notes</p>
                    <p className="text-[11px] text-gray-400 mt-0.5">{notes.length} note{notes.length !== 1 ? 's' : ''}</p>
                  </div>
                </motion.button>
              </div>

              {/* Article count */}
              <div className="mt-8 px-5 py-3 bg-white/80 backdrop-blur-sm rounded-full border border-gray-200 shadow-sm">
                ...
                {filteredItems.slice(0, itemsLimit).map(item => (
                  <div key={item.id} onClick={() => setSelectedItem(item)} className="bg-white rounded-xl border border-gray-200 p-4 flex items-center justify-between active:scale-[0.98] transition-transform cursor-pointer group">
                    <div className="flex-1 min-w-0 pr-4">
                      <h3 className="font-bold text-gray-900 line-clamp-2 group-hover:text-blue-600">{item.name}</h3>
          ...
          {view === 'notes' && (
            <motion.div key="notes" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="space-y-5">
              <div className="flex items-center justify-between">
                <button onClick={() => setView('home')} className="flex items-center gap-2 text-blue-600 font-bold"><ArrowLeft size={20} /> Retour</button>
                <h2 className="text-xl font-bold">Bloc-notes</h2>
                <div className="w-20" />
              </div>
              ...
              <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-5 space-y-3">
                <input
                  ...
                  className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl outline-none font-bold text-gray-900 focus:border-amber-400 focus:ring-2 focus:ring-amber-100 transition-all"
                />
                <textarea
                  ...
                  rows={4}
                  className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl outline-none text-gray-700 resize-none focus:border-amber-400 focus:ring-2 focus:ring-amber-100 transition-all"
                />
                ...
                <div className="space-y-2">
                  {notes.map(note => (
                    <motion.div
                      ...
                      animate={{ opacity: 1, y: 0 }}
                      className={cn(
                        "bg-white rounded-xl border p-4 shadow-sm transition-all cursor-pointer group",
                        editingNote?.id === note.id ? "border-amber-400 ring-2 ring-amber-100" : "border-gray-200 hover:border-amber-200"
                      )}
                    >
                      <div className="flex items-start justify-between gap-3">
                        <div className="flex-1 min-w-0" onClick={() => startEditNote(note)}>
                          <h3 className="font-bold text-gray-900 line-clamp-1 group-hover:text-amber-600 transition-colors">{note.title}</h3>
                          {note.content && <p className="text-sm text-gray-500 mt-1 line-clamp-2 whitespace-pre-wrap">{note.content}</p>}
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 p-4 flex justify-around sm:hidden z-40 shadow-[0_-4px_10px_rgba(0,0,0,0.03)]">
        <button onClick={() => setView('home')} className={cn("p-2 transition-colors", view === 'home' ? "text-blue-600" : "text-gray-400")}><Home /></button>
        <button onClick={() => setView('list')} className={cn("p-2 transition-colors", view === 'list' ? "text-blue-600" : "text-gray-400")}><Search /></button>
