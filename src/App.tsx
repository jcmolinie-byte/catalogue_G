...
export default function App() {
  ...
  const scoreItem = (item: CatalogItem, ai: AIAnalysis): number => {
      ...
      ai.specs.forEach(spec => {
        ...
        if (specClean.includes('kw')) {
          ...
          if (kwValue && itemPowerText.includes(`${kwValue}kw`)) {
            score += 3000; // Un match sur la puissance est prioritaire
          }
        }

        const commonSpec = ['50hz', '60hz', '230v', '240v', '380v', '400v', '415v', '460v'].includes(specNoSpace);
    ...
    if (item.sapCode && item.sapCode !== 'N/A') {
      ...
      if (sapStr.length >= 5) {
        const aiRawDesc = cleanString(`${ai.raw_ocr || ''} ${ai.model} ${ai.description} ${(ai.specs || []).join(' ')}`);
