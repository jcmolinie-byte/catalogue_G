...
export default function App() {
  ...
  const findCatalogItemBySap = (sapCode?: string) => {
    ...
    return catalogItemsRef.current.find(item => {
  ...
  const findCompanionResults = (question: string): CompanionResult[] => {
    ...
    const stopWords = new Set([
      'trouve', 'moi', 'dans', 'sur', 'avec', 'pour', 'les', 'des', 'une', 'un', 'le', 'la', 'du', 'de',
      'quel', 'quelle', 'quels', 'quelles', 'sont', 'caracteristiques', 'caracteristique', 'article',
      'poste', 'technique', 'ref', 'reference', 'sap', 'est', 'ce', 'cet', 'cette'
    ]);
    const articleTokens = tokens.filter(token =>
      token.length > 2 &&
      !stopWords.has(token) &&
      !equipmentLikeTokens.includes(token)
    );

    const expandToken = (token: string) => {
      if (token === 'debimetre' || token === 'debitmetre') return ['debimetre', 'debitmetre', 'debit', 'flowmeter'];
      if (token === 'capteur') return ['capteur', 'sensor'];
      if (token === 'pompe') return ['pompe', 'pump'];
      return [token];
    };
    const expandedArticleTokens = articleTokens.flatMap(expandToken);

    const equipmentPool = equipments.filter(part => {
      if (!normalizedEquipmentCode && equipmentLikeTokens.length === 0) return true;
      const compactEquipment = normalizeForCompanion(part.equipment).replace(/\s/g, '');
      const compactLabel = normalizeForCompanion(part.equipmentLabel).replace(/\s/g, '');
    ...
    const equipmentResults = equipmentPool.map(part => {
      const catalogItem = findCatalogItemBySap(part.sapCode);
      const searchable = normalizeForCompanion(`${part.designation} ${part.sapCode} ${part.equipment} ${part.equipmentLabel} ${catalogItem?.name || ''} ${catalogItem?.category || ''}`);
      let score = 0;

      if (normalizedEquipmentCode && normalizeForCompanion(part.equipment).replace(/\s/g, '').includes(normalizedEquipmentCode)) score += 100;
      expandedArticleTokens.forEach(token => {
        if (searchable.includes(token)) score += token.length > 4 ? 45 : 25;
      });
    ...
    const catalogResults = catalogItemsRef.current.map(item => {
      const searchable = normalizeForCompanion(`${item.name} ${item.sapCode} ${item.category} ${item.location}`);
      let score = 0;
      expandedArticleTokens.forEach(token => {
        if (searchable.includes(token)) score += token.length > 4 ? 35 : 20;
      });
  ...
  const buildFallbackCompanionAnswer = (question: string, results: CompanionResult[]) => {
    if (results.length === 0) {
      return "Je n'ai pas trouvé de correspondance fiable dans le catalogue ou les postes techniques. Essayez avec un code de poste, un code SAP, une désignation ou une marque visible sur la pièce.";
    }
  ...
  const askCompanionWithGemini = async (question: string, results: CompanionResult[]) => {
    const geminiKey = localStorage.getItem('gemini_api_key') || process.env.GEMINI_API_KEY;
    if (!geminiKey) return buildFallbackCompanionAnswer(question, results);
...
Ne pretend pas avoir consulte internet directement.`
    ...
    if (!response.ok) return buildFallbackCompanionAnswer(question, results);
    const data = await response.json();
    return data.candidates?.[0]?.content?.parts?.[0]?.text?.trim() || buildFallbackCompanionAnswer(question, results);
  };
  ...
  const submitCompanionQuestion = async (questionOverride?: string) => {
    ...
    try {
      const results = findCompanionResults(question);
      const sources = buildCompanionSources(question, results[0]);
      const answer = await askCompanionWithGemini(question, results);
