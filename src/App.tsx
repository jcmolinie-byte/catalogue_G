...
export default function App() {
  ...
  const scoreItem = (item: CatalogItem, ai: AIAnalysis): number => {
    ...
    if (ai.model) {
      ...
      // On cherche d'abord la référence complète exacte (très puissant)
      if (modelClean.length > 2 && searchableNoSpace.includes(modelClean)) {
        score += 2000;
      } else {
        // Sinon on cherche les parties du modèle (ex: "K37" dans "SEW K37")
        const modelParts = cleanString(ai.model).split(/[\s\-_/]+/);
        let partsMatched = 0;
        modelParts.forEach(p => {
          if (p.length > 2 && searchableText.includes(p)) partsMatched++;
        });
        score += (partsMatched * 400); // 400 points par bout de référence trouvé
      }
    }
    ...
    if (Array.isArray(ai.specs)) {
      ai.specs.forEach(spec => {
        ...
        if (specNoSpace.length > 1 && searchableNoSpace.includes(specNoSpace)) {
          score += 100; // Les caractéristiques techniques donnent des points bonus
        }
      });
    }
