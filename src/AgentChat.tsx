...
function detectQuestionType(query: string): 'catalog' | 'technical' | 'summary' {
  ...
  const summaryTriggers = [
    'avons-nous','avez-vous','est-ce qu','qu\'est-ce','qu\'avons','qu\'avez',
    'liste','inventaire','tout ce','ce qu\'on a','ce que vous avez',
    'ce que nous avons','disponible','en stock','stock de',
  ];
  if (summaryTriggers.some(t => q.includes(t))) return 'summary';

  const technicalTriggers = [
    'procédure','comment','monter','démonter','remplacer','diagnostiquer',
    'régler','aligner','consigner','déconsigner','graisser','lubrifier',
    'serrage','couple','vibration','température','bruit','fuite','panne',
    'loto','permis','sécurité','norme','iso','nf','schéma','plan',
  ];
...
function buildSystemPrompt(
  ...
  const basePrompt = `Tu es MAXI, expert en maintenance industrielle sur le site sucrier de Nesle (Tereos).
          ...
          >
            ...
            <div className="flex items-center gap-3 px-4 py-3.5 bg-blue-600 flex-shrink-0">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-blue-100 text-[11px]">Expert Maintenance Nesle</p>
              </div>
              {filterInfo && (
