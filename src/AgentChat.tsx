...
import { Send, ChevronDown, Loader2, Mic, Volume2, VolumeX, X } from 'lucide-react';
...
// ─── Recherche postes techniques dans Supabase ──────────────────────────────
async function searchEquipments(query: string): Promise<EquipmentItem[]> {
  const words = query
    .toLowerCase()
    .normalize('NFD').replace(/[\u0300-\u036f]/g, '')
    .split(/\s+/)
  ...
  // Cherche chaque mot dans les colonnes pertinentes
  const filters = words.map(w =>
    `equipment.ilike.%${w}%,equipment_label.ilike.%${w}%,designation.ilike.%${w}%,sap_code.ilike.%${w}%`
  ).join(',');
...
function buildSystemPrompt(
  query: string,
  catalogItems: CatalogItem[],
  equipmentResults: EquipmentItem[]
): { prompt: string; mode: string } {

  const questionType = detectQuestionType(query);

  const equipLines = equipmentResults.length > 0
    ? equipmentResults
...
## Postes techniques Nesle (${equipmentResults.length} postes pertinents trouvés sur 51 701 au total)
...
export default function AgentChat({ catalogItems, equipments, isDark }: AgentChatProps) {
  ...
  // Nettoie le markdown pour obtenir du texte lisible
  ...
  // Browser TTS amélioré (fallback)
  const speakWithBrowser = (text: string): Promise<void> => {
    return new Promise((resolve) => {
      const synth = window.speechSynthesis;
      synth.cancel();

      // Découper en phrases pour un rendu plus naturel
      const sentences = text
        .split(/(?<=[.!?:;])\s+/)
        .filter(s => s.trim().length > 0);

      const voices = synth.getVoices();
      // Préférer les voix haute qualité françaises
      const preferredVoice =
        ...
        voices.find(v => v.lang === 'fr-FR' && !v.localService) || // Voix réseau (meilleure qualité)
  ...
  const speakMessage = async (text: string, index: number) => {
    // Si on clique sur le même message qui parle, on arrête
    ...
    // Essayer ElevenLabs d'abord, puis navigateur
    const elevenLabsOk = await speakWithElevenLabs(cleanText);
    if (!elevenLabsOk) {
  ...
  // Arrêter la lecture quand on ferme le chat
  useEffect(() => {
    if (!isOpen) {
      window.speechSynthesis?.cancel();
      if (audioRef.current) {
  ...
  const sendMessage = async (text: string) => {
    ...
    try {
      // Recherche les postes techniques pertinents dans Supabase
      const equipResults = await searchEquipments(trimmed);
      const { prompt, mode } = buildSystemPrompt(trimmed, catalogItems, equipResults);
      setFilterInfo(mode);
      ...
      <AnimatePresence>
        {!isOpen && (
          <motion.button
            ...
            title="SAM - Stock Assistant Maintenance"
          ...
          >
            ...
            <div className="flex items-center gap-3 px-4 py-3.5 bg-blue-600 flex-shrink-0">
              ...
              <div className="flex-1 min-w-0">
                <p className="font-bold text-white text-sm leading-tight">SAM</p>
                <p className="text-blue-100 text-[11px]">Stock Assistant Maintenance</p>
              </div>
              {filterInfo && (
              ...
              <button
                ...
                className="w-8 h-8 bg-red-500 hover:bg-red-600 rounded-xl flex items-center justify-center transition-colors"
              >
                    ...
                    {msg.role === 'user'
                      ...
                      : <>
                          ...
                          <button
                            ...
                            className={cn(
                              "mt-2 flex items-center gap-1 text-[11px] px-2 py-1 rounded-lg transition-colors",
                              speakingIndex === i
            ...
            <div className={cn(
              ...
              <div className={cn(
                ...
                <button
                  ...
                  className={cn(
                    "w-8 h-8 rounded-xl flex items-center justify-center transition-all flex-shrink-0 active:scale-90",
                    isListening
              ...
              <p className={cn(
                ...
                Entrée pour envoyer · 🎙️ Micro pour dicter
              </p>
            </div>
          </motion.div>
        )}
